...
resulting horizon law.  The same relation can be read in the opposite
direction.  Given a desired one-sided visibility distance as a function of
endpoint height, the corresponding stratified refractive-index profile can be
reconstructed directly.  This inverse formulation separates the mathematical
question of existence from the physical question of whether the reconstructed
profile can occur in an atmosphere.
...
\end{equation}
The integrand depends only on the local value of the refractive index.
Differentiating Eq.~\eqref{eq:inverse-forward-relation} with respect to its
upper limit yields
\begin{equation}
    d'(h)
    =
...
\end{equation}
Thus any differentiable, strictly increasing horizon law with $d'(h)>0$
defines a refractive-index profile within the assumptions of the model.  The
...
\end{equation}
By construction, a grazing ray in this stratified medium above a flat opaque
boundary has exactly the same one-sided distance--height relation as a tangent
...
The reconstruction concerns the limiting grazing distance and therefore the
associated boundary-occlusion criterion.  It does not by itself establish
