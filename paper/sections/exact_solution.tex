...
order. Figure~\ref{fig:exact-paraxial-comparison} compares the exact
normalized trajectory with its paraxial approximation and shows how the
relative paraxial error grows outside the small-slope regime.

\begin{figure}[htbp]
    \centering
    \input{figures/exact_paraxial_comparison}
    \caption{Comparison of the exact exponential-model ray with its paraxial
    approximation, using $X=\kappa(x-x_m)$ and
    $H=\kappa(h-h_m)$. (a) Exact and paraxial normalized trajectories.
    (b) Relative paraxial error
    $\varepsilon_{\mathrm p}=100(H_{\mathrm e}-H_{\mathrm p})/H_{\mathrm e}$,
    together with its small-$X$ approximation $100X^2/6$.}
    \label{fig:exact-paraxial-comparison}
\end{figure}

The exact curvature is not constant away from the minimum. Indeed,
