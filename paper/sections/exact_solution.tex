...
normalized trajectory with its paraxial approximation and shows how their
difference grows outside the small-slope regime.

\begin{figure}[htbp]
    \centering
    \input{figures/exact_paraxial_comparison}
    \caption{Comparison of the exact exponential-model ray with its paraxial
    approximation. (a) Normalized grazing trajectories
    $H_{\mathrm e}(X)=-\ln(\cos X)$ and $H_{\mathrm p}(X)=X^2/2$.
    (b) Their difference $\Delta H=H_{\mathrm e}-H_{\mathrm p}$, together
    with the leading local term $X^4/12$.}
