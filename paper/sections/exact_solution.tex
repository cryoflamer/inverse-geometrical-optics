...
\end{equation}
as the natural candidate for reproducing spherical horizon behaviour above a
flat boundary.  Because the stratified problem possesses the first integral
introduced in Sec.~\ref{sec:model}, this profile can also be
solved without the small-slope approximation.
...
\end{equation}
so the parabolic result of the previous section is recovered at leading
...
\end{equation}
Thus the exponential profile produces genuine geometric occlusion above a
flat surface: beyond the limiting separation, every connecting ray intersects
...
\end{equation}
The first term is exactly the paraxial distance
